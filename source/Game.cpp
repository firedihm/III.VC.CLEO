...
namespace game
{
		...
		ProcessScript_ft* ProcessScript = gaddr<ProcessScript_ft*>(Address::ProcessScript);
		...
		float* TimeStep = gaddr<float*>(Address::TimeStep);
		...
		bool make_hooks = []() {
				...
				memory::make_call(gaddr(Address::InitScripts_call0), &OnGameStart);
				memory::make_call(gaddr(Address::InitScripts_call1), &OnGameLoad);
				memory::make_call(gaddr(Address::InitScripts_call2), &OnGameReload);
				memory::make_call(gaddr(Address::SaveAllScripts_call), &OnGameSaveAllScripts);
				memory::make_call(gaddr(Address::CdStreamRemoveImages_call), &OnGameShutdown);
				...
				*/
				...
				memory::make_jump(gaddr(Address::GetText), &fxt::find);

				memory::make_jump(gaddr(Address::InitScript), Script::gaddr(&Script::Init));
				memory::make_jump(gaddr(Address::ProcessOneCommand), Script::gaddr(&Script::ProcessOneCommand));
				memory::make_jump(gaddr(Address::CollectParameters), Script::gaddr((int(Script::*)(uint*, short))& Script::CollectParameters)); // we have to specify overload here
				memory::make_jump(gaddr(Address::CollectNextParameterWithoutIncreasingPC), Script::gaddr(&Script::CollectNextParameterWithoutIncreasingPC));
				memory::make_jump(gaddr(Address::GetPointerToScriptVariable), Script::gaddr(&Script::GetPointerToScriptVariable));
