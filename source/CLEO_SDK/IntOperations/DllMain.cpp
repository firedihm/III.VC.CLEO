...
eOpcodeResult __stdcall
BIT_AND_COMPOUND(Script* script)
{
		ScriptParam* param = script->GetPointerToScriptVariable();
		script->CollectParameters(1);

		param->nVar &= game::ScriptParams[0].nVar;

		return OR_CONTINUE;
}

eOpcodeResult __stdcall
BIT_OR_COMPOUND(Script* script)
{
		ScriptParam* param = script->GetPointerToScriptVariable();
		script->CollectParameters(1);
...
eOpcodeResult __stdcall
BIT_XOR_COMPOUND(Script* script)
{
		ScriptParam* param = script->GetPointerToScriptVariable();
		script->CollectParameters(1);
...
eOpcodeResult __stdcall
BIT_NOT_COMPOUND(Script* script)
{
		ScriptParam* param = script->GetPointerToScriptVariable();
...
eOpcodeResult __stdcall
BIT_SHR_COMPOUND(Script* script)
{
		ScriptParam* param = script->GetPointerToScriptVariable();
		script->CollectParameters(1);
...
eOpcodeResult __stdcall
BIT_SHL_COMPOUND(Script* script)
{
		ScriptParam* param = script->GetPointerToScriptVariable();
		script->CollectParameters(1);
...
APIENTRY DllMain(HMODULE hModule, DWORD reason_for_call, LPVOID reserved)
{
		if (reason_for_call == DLL_PROCESS_ATTACH) {
				...
				opcodes::reg(0x0B10, BIT_AND);
				opcodes::reg(0x0B11, BIT_OR);
				opcodes::reg(0x0B12, BIT_XOR);
				opcodes::reg(0x0B13, BIT_NOT);
				opcodes::reg(0x0B14, MOD);
				opcodes::reg(0x0B15, BIT_SHR);
				opcodes::reg(0x0B16, BIT_SHL);
				opcodes::reg(0x0B17, BIT_AND_COMPOUND);
				opcodes::reg(0x0B18, BIT_OR_COMPOUND);
				opcodes::reg(0x0B19, BIT_XOR_COMPOUND);
				opcodes::reg(0x0B1A, BIT_NOT_COMPOUND);
				opcodes::reg(0x0B1B, MOD_COMPOUND);
				opcodes::reg(0x0B1C, BIT_SHR_COMPOUND);
				opcodes::reg(0x0B1D, BIT_SHL_COMPOUND);
		}
