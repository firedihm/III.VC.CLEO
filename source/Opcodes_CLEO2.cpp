...
SPAWN_VEHICLE_BY_CHEATING(Script* script)
{
		...
		if (!script->is_III_) {
				...
				/*
				 *	Default game had "model" argument pushed with 2 byte instructions, because MI_RHINO < 127. 
				 *	But we are very lucky that TankCheat() has duplicated CHud::SetHelpMessage() call: 
				 *	we can patch out one of them to preload new model into callee-saved register EDI 
				 *	and push it in place of short 2 byte instruction.
				 */
				memory::write(func + 0x18, uchar(0xBF));    // mov   edi,model
				memory::write(func + 0x19, model);
				memory::write(func + 0x21, ushort(0x9057)); // push  edi, nop	;arg for CStreaming::RequestModel()
				memory::write(func + 0x33, orig_model_load_state + (model - orig_model) * 0x14); // CStreaming::ms_aInfoForModel[model].m_loadState
				memory::write(func + 0xA4, ushort(0x9057)); // push  edi, nop	;arg for CAutomobile::CAutomobile()

				game::SpawnCar(model);

				memory::call(func + 0x18, game::SetHelpMessage);
				memory::write(func + 0x21, ushort(0x7A6A)); // push  MI_RHINO	;arg for CStreaming::RequestModel()
				memory::write(func + 0x33, orig_model_load_state); // CStreaming::ms_aInfoForModel[MI_RHINO].m_loadState
				memory::write(func + 0xA4, ushort(0x7A6A)); // push  MI_RHINO	;arg for CAutomobile::CAutomobile()
		}
