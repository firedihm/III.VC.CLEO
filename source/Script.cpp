		...
		for (short i = 0; i < num_params; i++) {
				...
				case ScriptParam::Type::String:
						...
						*p_ip += std::strlen((char*)&game::ScriptSpace[*p_ip]) + 1;
						break;
				default:
						// text label: there was no data type prefix
						(*p_ip)--;
						game::ScriptParams[i].szVar = (char*)&game::ScriptSpace[*p_ip];
...
Script::scan_string(const char* in, const char* format, bool return_packed_data)
{
		...
		while (*format) {
				if (std::isspace(*format)) {
						...
						bool ignore_first_param = conv_spec[1] == '*' || (conv_spec[0] == '%' && conv_spec[1] == '%');

						// params are passed as binaries: no type-checking; second param is redundant if check above passes
						num_assigned += std::sscanf(in, conv_spec, (ignore_first_param ? &chars_read : (int*)GetPointerToScriptVariable()), &chars_read);
						in += chars_read;
				}
		}
