...
namespace game
{
		...
		using DoDeathArrestCheck_ft = void __fastcall(Script*, int); // __thiscall

		using SearchText_ft = wchar_t* __fastcall(void*, int, const char* key); // __thiscall
		using AddMessage_ft = void __cdecl(const wchar_t* key, uint time, ushort pos);
		using AddMessageJumpQ_ft = void __cdecl(const wchar_t* key, uint time, ushort pos);
		using AddBigMessageQ_ft = void __cdecl(const wchar_t* key, uint time, ushort pos);
		using SetHelpMessage_ft = void __cdecl(const wchar_t* message, bool quick, bool display_forever); // last param is used only in VC
		...
		extern Script** ppIdleScripts; // list of unused elements of game::ScriptsArray
		extern Script** ppActiveScripts; // list of used elements of game::ScriptsArray and custom scripts processed by game
		extern ushort* pCommandsExecuted;
		extern ushort* pScriptsUpdated;
		extern bool* pAlreadyRunningAMissionScript; // rather useless flag; it was probably meant to prevent >1 missions loading at once
		extern bool* pDbgFlag;
		extern uchar* pFailCurrentMission;
		...
		extern DoDeathArrestCheck_ft* DoDeathArrestCheck;
		...
		extern short* pPadOldState;
		extern bool* pWideScreenOn;
		extern short* pOldWeatherType;
		extern char* RootDirName;
		extern uint* pTimeInMillisecondsPauseMode;
		extern uint* pTimeInMilliseconds;
		extern float* pTimeStep;
		extern bool* pGameNotLoaded;
		...
		// memory expansion
		extern Script* ScriptsArray; // for generic .scm scripts; .cs are stored in script manager
